...
from geodataimport._testing import skip_on_exception
from geodataimport.utils import RemoteDataError
from geodataimport.wb import WB, download, get_indicators, search, wb_get_countries
...
class TestWB(object):
    def test_wdi_search(self):
        ...
        result2 = WB().search("gdp.*capita.*constant")
        session = requests.Session()

        result3 = search("gdp.*capita.*constant", session=session)
        result4 = WB(session=session).search("gdp.*capita.*constant")
    ...
    def test_wdi_wb_get_countries(self):
        result1 = wb_get_countries()
        result2 = WB().wb_get_countries()

        session = requests.Session()
        result3 = wb_get_countries(session=session)
        result4 = WB(session=session).wb_get_countries()
    ...
    def test_wdi_get_indicators(self):
        ...
        result2 = WB().get_indicators()

        session = requests.Session()
        result3 = get_indicators(session=session)
        result4 = WB(session=session).get_indicators()
